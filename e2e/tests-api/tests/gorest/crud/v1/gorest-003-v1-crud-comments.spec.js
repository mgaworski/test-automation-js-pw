...
import { compare_comments } from '../../../../clients/gorest/gorestClient.js';
...
test.describe.serial('GoRest API V1 CRUD: Posts', () => {
    ...
    test('CRUD Comment - #0 - Preparation (V1) - getting existing post ID as parent', async ({ request }) => {
    ...
    test('CRUD Comment - #1 - CREATE new Comment (V1)', async ({ request }) => {
        const gorestClient = new GorestClient(request, cfg, version, process.env.GOREST_TOKEN);
        comment.post_id = articleId;
        const response = await gorestClient.create_comment(comment);
        expect(response.ok()).toBeTruthy();
        const newComment = (await response.json()).data;
        expect(compare_comments(newComment, comment)).toBeTruthy();
        commentId = newComment.id;
    });

    test('CRUD Comment - #2 - READ previously created Comment (V1)', async ({ request }) => {
        const gorestClient = new GorestClient(request, cfg, version, process.env.GOREST_TOKEN);
    ...
    test('CRUD Comment - #3 - UPDATE previously created Comment (V1)', async ({ request }) => {
    ...
    test('CRUD Comment - #4 - DELETE previously created Comment (V1)', async ({ request }) => {
        const gorestClient = new GorestClient(request, cfg, version, process.env.GOREST_TOKEN);
